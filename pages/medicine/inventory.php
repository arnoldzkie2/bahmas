...
$query = "SELECT * FROM inventory LIMIT $start, $entries";
$result = mysqli_query($con, $query);
$total_item = mysqli_num_rows(mysqli_query($con, "SELECT * FROM inventory"));
...
if(isset($_GET['print'])) {
    ...
    header('Content-Disposition: attachment; filename="child.csv"');
      ...
      $index = $start + 1;
      $total_data = "select * from inventory";
      $result_data = mysqli_query($con, $total_data);
      while($row = mysqli_fetch_assoc($result_data)) {
          $line = array($index, $row['item'], $row['description'], $row['quantity'], $row['arrival'], $row['expiry']);
          fputcsv($output, $line);
          $index++;
      }
      fclose($output);
...
<h1><i class="fa-solid fa-kit-medical" ></i> Medicine Inventory</h1>
<div class="content">
    <form class="option">
          <a href="./new-item.php">Add New</a>
          <button name='print' class='print'>Print</button>
    </form>
<?php
?>
<div class="display">
  <h2>Total number of medicine <span><?php echo $total_item ?></span></h2>
  <div class="features">
    <div class="entries">
      Show 
    ...
    <div class="search">
      <input type="text" placeholder='Search medicine' oninput="filterTable()"><i class="fa-solid fa-magnifying-glass" onclick="filterTable()"></i>
    </div>
  </div>
  ...
  <tbody>
    ...
    ?>
      <tr>  
        ...
        <td><?php echo $row['arrival'] ?></td> <!-- Civil status -->
        <td><?php echo $row['expiry'] ?></td> <!-- Birthday -->
      </tr>
      <?php $index++; ?>
    <?php endwhile; ?>
  </tbody>
...
<script>
  ...
  window.location.href = `../../routes/medicine/inventory-info.php?id=${id}`;
}
